...
(defclass triangle-window (glut:window)
  ((vbo :accessor vertex-buffer)
   (vs :accessor vertex-shader)
   (fs :accessor fragment-shader)
   (vao :accessor vertex-array)
   (program :accessor program)
   (angle :accessor angle :initform 0.0)) 
  (:default-initargs :width 600 :height 400 :pos-x 100 :pos-y 100
		     :mode '(:double :rgb :depth) :title "01-triangle.lisp"))

(defparameter *triangle-vertex-shader-program*
  "
#version 330 core
...
(defparameter *triangle-fragment-shader-program*
  "#version 330 core
...
(defmethod glut:display-window :before ((w triangle-window))
  ...
  (let ((vs (gl:create-shader :vertex-shader))
    ...
    (gl:shader-source vs *triangle-vertex-shader-program*)
    (gl:compile-shader vs)
    (gl:shader-source fs *triangle-fragment-shader-program*)
...
(defmethod glut:display ((w triangle-window))
    (update-swank)
...
(defmethod glut:reshape ((w triangle-window) width height)
...
(defmethod glut:keyboard ((w triangle-window) key x y)
  (declare (ignore x y))
  (case key
...
(defmethod glut:close ((w triangle-window))
...
(defun 01-triangle ()
  (let ((w (make-instance 'triangle-window)))
    (unwind-protect
         (continuable
